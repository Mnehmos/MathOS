...
- Draft PR #22 carries the first hosted publication-boundary smoke. Runs `29706858126` through `29707579646` progressively exposed missing isolation software, hidden toolchain lookup, namespace identity, path traversal, and read-only mount assumptions. Runs `29707668753` and `29707749539` reached Lean inside the isolated namespace and showed that default host-sized thread creation exceeds even a 4 GiB address-space ceiling. The publication invocation now fixes Lean concurrency at one thread and records that choice alongside the enforced 4 GiB limit.
