...
- Draft PR #22 carries the first hosted publication-boundary smoke. GitHub Actions run `29706858126` correctly failed closed because Bubblewrap was absent from the runner image. Run `29706988640` prompted exact Lean binary resolution and retained-stderr reporting. Runs `29707138501`, `29707309208`, `29707401642`, and `29707483900` exposed hosted-runner traversal restrictions across namespace identities, while `29707213042` showed that fully unprivileged setup cannot configure the isolated loopback device. The ephemeral runner now grants traverse-only access to the exact checkout and toolchain paths before binding them read-only, and every Linux capability is dropped before Lean executes.
