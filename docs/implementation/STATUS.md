...
1. Create authoritative exact proof/refutation evidence only through an atomic application gate that fully replays a current `PublicationIngestionReceiptSnapshot` and its bound stage, raw verifier output, and canonical CAS attestation-verification record, requires a `passed` report, and never trusts receipt-table presence or caller-authored reports.
2. Derive mathematical status only from exact current proof and fidelity evidence.
3. Complete Pilot A through the real interfaces only after both authority and fidelity controls exist.
