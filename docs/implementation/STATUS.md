...
- Draft PR #22 carries the first hosted publication-boundary smoke. GitHub Actions run `29706858126` correctly failed closed because Bubblewrap was absent from the runner image. Run `29706988640` prompted exact Lean binary resolution and retained-stderr reporting. Run `29707138501` then proved that invoking Bubblewrap through `sudo` discarded the runner user's access to the checkout. The boundary now uses Bubblewrap's unprivileged user namespace, preserving access without adding privilege.
