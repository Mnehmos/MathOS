...
1. Land the controlled-ingestion slice through protected review and audit the exact retained stage, raw verifier output, canonical CAS attestation-verification record, and non-authoritative SQLite ingestion receipt from the merged `main` workflow.
2. Create authoritative exact proof/refutation evidence only through an atomic application gate that fully replays a current `PublicationIngestionReceiptSnapshot` and its bound stage, raw verifier output, and canonical CAS attestation-verification record, requires a `passed` report, and never trusts receipt-table presence or caller-authored reports.
