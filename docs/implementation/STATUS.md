...
- Draft PR #22 carries the first hosted publication-boundary smoke. GitHub Actions run `29706858126` correctly failed closed because Bubblewrap was absent from the runner image. Run `29706988640` prompted exact Lean binary resolution and retained-stderr reporting. Runs `29707138501`, `29707309208`, and `29707401642` exposed hosted-runner path traversal across namespace identity changes, while `29707213042` showed that fully unprivileged setup cannot configure the isolated loopback device. Privileged setup now binds the exact checkout and toolchain to neutral read-only paths, then drops every Linux capability before the verifier executes.
