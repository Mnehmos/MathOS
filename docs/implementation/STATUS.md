...
- Draft PR #22 carries the first hosted publication-boundary smoke. GitHub Actions run `29706858126` correctly failed closed because Bubblewrap was absent from the runner image. Run `29706988640` prompted exact Lean binary resolution and retained-stderr reporting. Run `29707138501` exposed checkout ownership across privileged setup. Run `29707213042` showed that fully unprivileged setup cannot configure the isolated loopback device on the hosted kernel. Bubblewrap now receives privilege only to construct namespaces and explicitly drops execution back to the runner UID and GID before Lean starts.
