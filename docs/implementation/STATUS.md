...
- Draft PR #22 carries the first hosted publication-boundary smoke. GitHub Actions run `29706858126` correctly failed closed because Bubblewrap was absent from the runner image. Run `29706988640` prompted exact Lean binary resolution and retained-stderr reporting. Runs `29707138501`, `29707309208`, `29707401642`, and `29707483900` exposed hosted-runner traversal restrictions across namespace identities. Run `29707579646` reached mount construction and exposed nonexistent destinations below the read-only root. Exact inputs now replace existing neutral `/mnt` and `/opt` mount points, with every capability dropped before Lean executes.
