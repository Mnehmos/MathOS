...
- Draft PR #22 carries the first hosted publication-boundary smoke. GitHub Actions run `29706858126` correctly failed closed because Bubblewrap was absent from the runner image. Run `29706988640` prompted exact Lean binary resolution and retained-stderr reporting. Runs `29707138501` and `29707309208` exposed checkout traversal after namespace privilege changes, while `29707213042` showed that fully unprivileged setup cannot configure the isolated loopback device. The boundary now binds the exact checkout and resolved Lean toolchain to neutral in-namespace paths before dropping to the runner UID and GID.
