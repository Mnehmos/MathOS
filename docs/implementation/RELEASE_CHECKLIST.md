...
Phase 3 publication contract evidence: the closed publication policy, request, and candidate report bind exact proof or refutation intent, diagnostic and audit evidence, formalization, environment, module, declaration, Git commit and tree, protected workflow identity, runner class, required isolation controls, retained artifacts, and SHA-pinned attestation actions. The application now derives canonical private request artifacts from the current formalization head and one exact accepted elaboration/audit chain, recomputes every retained local CAS object, and exposes the same non-authoritative operation through CLI and MCP without accepting request JSON or authority fields. Typed formalization polarity prevents one exact version/evidence closure from being requested as both proof and refutation; legacy records without polarity cannot publish until versioned and reverified. Transactional head and idempotency checks cover matching preexisting CAS identities. Retry, restart, stale subject, conflicting key reuse, missing/substituted evidence, corrupted evidence artifacts, and invalid or mismatched outcomes fail closed. Candidate reports are structurally non-authoritative. Polarity remains intent until combined with reviewed fidelity. ADR-0006 requires OIDC and Sigstore attestation verification before promotion. A real protected candidate closure, verified ingestion, and authoritative evidence remain incomplete, so every proof-authority criterion remains unchecked.
