...
Pilot A refutation evidence: issue #41 and PR #42 commit an exact no-import Lean module for “Every prime number is odd,” including a named checked witness that `2` is prime and not odd and a negation theorem that uses it. The protected producer creates the exact source, normalized universal natural-number claim, negation-polarity formalization, local evidence, and refutation request through `mcl`; its staging, ingestion, authority, v2 logical-negation fidelity, and derived-status path remains unchanged. A Windows CLI playtest completed elaboration and axiom-free audit, prepared the refutation request, and rejected a proof request for the same formalization. Exact-head CI, merged-tree CI, and the protected publication run passed; the retained artifact was independently audited through authoritative refutation, distinct-reviewer logical-negation fidelity, and the derived `disproved` read. This satisfies the exact proof-and-refutation evidence item only. The broad lifecycle and Pilot A boxes remain open until counterexample packaging, immutable repair, repaired proof, pedagogy, and export are implemented.

## Verification

- [x] Lean verification uses pinned environments.
- [x] Authoritative proof and refutation evidence is recorded against exact formalization versions.
...
Remote Pilot A refutation evidence: PR #42 merged final reviewed head `1a5755fa815ca187c594e1d4db0c488d0a5e327c` as `18abdd23025cc245f0b5722a8676e5b3f1bb2237`, tree `8945fd4746c1f9d9d536dd14d17854da3c9d0bdd`. Exact-head PR run `29860329570` and main run `29860589159` passed all five required jobs. Protected publication run `29860588969`, job `88735987761`, retained the full refutation-authority, role-separated fidelity, and before/after status path. Artifact `8507149183` contains 78 files, is 89,176 archive bytes, and has GitHub and independently downloaded SHA-256 `7172aa87bccedf6a694299d076e6279061cd541cf5d4b8625d01bb7e0ba3b87f`.

The independent audit matched all archive members and 25 closure roles; parsed 54 JSON files plus 25 JSONL entries; verified exact committed module hash `1e011925f7444a635c263725bf5f7dfc6fabdcd17c6f7e97135b22a534cc5af3` and the empty axiom set; and recomputed request `513ed6ddb73294d03e8225298ef1f94b123434b52fa65da3540ead4a8ca88e7d`, report `0ab1aa3c7e46814b06d235987caa0b0fae7cf146ff717697ceafcb8ff625a106`, closure `85a72971c8715c7998492143a0e63db18fae06da7283d1d8be313c66d036f340`, bundle `2f82f4c28c76736ce65597608f33b9ddf719a6bc4dc213e80b92c8008b5aca9f`, stage `1ec7bbd2054173bba8cb310e66fbfc7dcff47171416ffd80ce0b02034197461d`, raw verification `089af2f16be942bbb58995df745165ce4bba2f4a6273fd7955446974484045df`, and receipt `46d88e197a040be6ce851fb28f980aee1f4dfda7714e304e5dd6e807405aaa52`. Accepted authoritative `lean_kernel_refutation` evidence `52484265b0cae459cca257b95cbec4f00033987929ff7394b3db3ffa547d9225` names exact negation formalization version `1f9621d15a83cbf3c201cd6d3cc739f4624cee59fdc103cf603b94f86ebac0b9`; distinct-reviewer `logical_negation` evidence `8d809395a76a5c23b126053d6b70e40a398d346a876b821ee25d9cbcf2119bfc` changes the exact claim read from `open/no_current_verified_fidelity` to `disproved` with one refutation witness. No Boolean authority assertion exists, exactly one class is authoritative, all eight stderr files are empty, and the attestation binds the exact protected workflow, commit, run, hosted runner, repository IDs, and Rekor timestamp `2026-07-21T19:14:47Z`.
...
- [x] MathCorpus and MCIP export works.
- [x] RL and evaluation exports work with leakage-aware splits.
- [x] Public exports fail closed on restricted or incomplete provenance.

Phase 6 portable-release evidence: issue [#50](https://github.com/Mnehmos/MathOS/issues/50) adds the closed Rust-owned `release_manifest/1` schema and a deterministic dry-run/new-directory builder rooted in one fully replayed publication receipt plus its persisted authority/current fidelity witness and one exact reviewed pedagogy path. Every required SPEC directory has manifest-bound members; exact record, logical/pedagogy edge, evidence, artifact, environment, publication, replay, report-copy, and license relationships are checked without SQLite. Private members retain explicit restrictions, and the public release profile rejects any restricted, private, or unlicensed member. Unit coverage reproduces the preview hash after restart, blocks overwrite and public projection, hides SQLite, revalidates the directory, and rejects extra files, expected-hash substitution, and a changed export. The protected Pilot A workflow builds five reviewed training-ineligible units, copies the resulting release, hides `state.sqlite3`, invokes verification with a nonexistent instance root and trusted manifest hash, replays the exact repaired theorem under Lean 4.32.0, and retains both bundle and report.

Phase 6b corpus-export evidence: issue [#54](https://github.com/Mnehmos/MathOS/issues/54) and PR [#55](https://github.com/Mnehmos/MathOS/pull/55) pin the byte-exact Apache-2.0 MathCorpus packet and minimum MCIP v1 schemas at upstream commit `a0d08c9ace0dcc70a8bc281dcf29c560242075d3`, add the closed Rust-owned `corpus_export_manifest/1` contract, and expose database-independent `mcl release export` and `verify-export`. Classification is explicit rather than guessed. Private releases produce only `private_audit_only` packets, `private_only` MCIP records, and private sensitive members; public releases remain quarantined and additionally require explicit public redistribution, redaction, and license authority. Verification checks a trusted export hash, exact inventory and schemas, canonical packet/record hashes, source/publication/environment/module bindings, and byte-identical deterministic reprojection from an independently supplied frozen release. Unit and CLI coverage blocks overwrite, missing-root/config coupling, invalid curation, extra files and directories, packet/schema substitution, and expected-hash substitution. Exact-head CI `29884823358`, merged-tree CI `29885040172`, and protected publication run `29885040178` passed. Downloaded artifact `8516235885` independently reproduced source release `7b002ffe0fcaeecd0fe745dcedd135442b7372985a0fac5b9ffbe03af4c691b1`, corpus manifest `4cb348d8a446f800d74865909c8fff66b310c5cd0fbe6fb00f6830d0961ed705`, and packet `690d7f8a18fad1fee415b5d6ad4efeaf154a2a7dc366b892967230d49db6c69d`; all 12 files and 11 manifest members matched, no links existed, deterministic reprojection passed without SQLite, and upstream MathCorpus and MCIP validators reported zero errors and warnings with hash checks enabled. RL/evaluation export and leakage-aware splits were still unfinished at that checkpoint; Phase 6c evidence follows.

Phase 6c leakage-aware RL/evaluation evidence: issue [#57](https://github.com/Mnehmos/MathOS/issues/57) and PR [#58](https://github.com/Mnehmos/MathOS/pull/58) add the closed Rust-owned plan, task, leakage-report, and export-manifest v1 contracts; receipt-bound chronology; required declared plus derived exact relationship keys; transitive component locking; private-held-out/public-split policy; six evidence-backed task families plus conditional pedagogy tasks; explicit family skips; and database-independent deterministic reprojection after the separate platform-bound release replay. Reviewed head `2d0c8fd0ec417ea0801d8995d4b93414fcf0a814` merged as `4b0a740b666e5e020b25e078ae80e1a6e7b9ab2b`. Exact-head CI `29889101827`, merged-tree CI `29889317590`, and protected publication run `29889317589` passed.

Downloaded artifact `8517692129`, `publication-boundary-4b0a740b666e5e020b25e078ae80e1a6e7b9ab2b`, is 470,757 archive bytes with GitHub SHA-256 `5598132289fe4a6c7d5527f5dcad4129e2a1048fd3f5ea2a912d72af1902197a`; it expands to 360 files and 885,321 bytes with no links. Protected Lean 4.32 replay and independent offline verification bind source release `47c72887ebb5421c021c1ed71212e4b4d298f8c02e3d4623223721c2a858f567`, canonical plan `0fe5bca2a96bff6c924cf837fa91a86b2714cf754c42cf459424f0f1d77ee9e8`, and RL export `fc0d4ccd3945aa3275a670dd5d367881c483c599bfa5350a604ed99a5ef3a5e5`. The exact 31-file export contains 30 manifest members, 102,369 member bytes, 10 private held-out tasks, one leakage component, and the six claimed families. Independent audit found zero inventory/hash/policy/trust-reference/cross-claim/cross-split/reasoning-field findings and reproduced every byte without SQLite. Decomposition, proof repair, generalization, frontier selection, Comparator, remaining pilots, migration, and full 1.0 acceptance remain separate open work.
