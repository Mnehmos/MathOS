...
The protected workflow refuses to start unless GitHub's immutable run context reports that `main` is actively protected. It constructs one fresh real canonical Pilot A state, then publishes two separately retained and attested candidates in causal order. The first candidate formalizes `MathOS.PilotA.every_prime_is_odd_refuted`, proves the explicit witness `2` is prime and not odd, and requests refutation authority for the logical negation of “Every prime number is odd.” Its controlled ingestion, role-separated fidelity review, derived `disproved` status, counterexample-search event, package, and atomic repair create a new independent claim for “Every prime number other than 2 is odd.” Only then may a child candidate in the same state formalize `MathOS.PilotA.every_prime_other_than_two_is_odd` at claim polarity and request proof authority. The child must load the exact repaired object/version from the retained repair, observe `not_started` with no inherited witnesses, and remain contained by the original instance root.

Each candidate rebuilds the exact no-import module inside Bubblewrap for elaboration, dependency discovery, and a verifier-controlled `#print axioms` driver targeted at its own declaration. The shared module defines the exact natural-number prime and odd predicates and both the refutation and repaired theorem, but the requests, formalizations, local evidence, reports, closures, attestations, receipts, authority evidence, and fidelity evidence all have separate immutable identities. The proof ingestion re-reads the original status and counterexample package after promotion and requires byte-identical results, while the repaired claim must derive `proved` from its own proof and `claim`-relation fidelity witnesses. All protected subprocesses use a read-only root, separate mount, PID, and network namespaces, one Lean worker thread, a 120-second timeout, a four-gibibyte Lean heap cap, and a six-gibibyte process address-space ceiling for runtime and thread overhead.
...
The closed policy, request, retained-closure, candidate-report, quarantine-stage, attestation-verification, authority-binding, authoritative-evidence, polarity-aware fidelity, derived-status, and counterexample-repair contracts are implemented. The shared application, CLI, and MCP request-preparation paths derive and retain a non-authoritative request from exact current local evidence without accepting caller-authored JSON. The protected `main` workflow retains the standalone boundary smoke and additionally produces, application-validates, attests, independently challenges, stages, ingests, promotes, and retains a real `publication_report/1`, its complete exact closure, raw verifier output, canonical CAS attestation-verification record, separate immutable non-authoritative SQLite ingestion receipt, receipt-bound authoritative evidence, role-separated fidelity review, and before/after derived-status reads. For the Pilot A refutation it then runs the controlled counterexample repair path and retains witness `2`, the canonical package, distinct repaired claim, repair edge, exact retry, retrieval replay, and unchanged-original/new-claim status reads. A second child candidate reuses that exact repaired object and version, obtains independent proof authority and `claim`-relation fidelity evidence, derives the repaired claim's `proved` status, and replays the original refutation status and repair package byte-for-byte to exclude inherited or mutated evidence. Pull-request CI exercises the same refutation candidate producer and quarantine persistence, plus direct elaboration and no-axiom checks for the repaired theorem, but its deliberately synthetic bundle cannot cross the ingestion or authority boundary or manufacture the protected repair required by the child candidate.

Historical protected proof authority is observed and independently audited on merge `0ef8a132dfb99e63238325ac035de948e893f791`, run `29726693055`, artifact `8454565233`. Derived source-claim status is observed and independently audited on merge `8e65549d40b443497651b7dced2dc72d1b31335e`, run `29856660579`, artifact `8505590232`: authority alone leaves the source claim open, and a distinct reviewer’s v2 `claim` fidelity evidence changes the derived read to `proved` without persisting a verdict.

The attestation-verification record and SQLite ingestion receipt remain non-authoritative. Together they can establish that exact retained report bytes were signed by the constrained protected workflow and witnessed by the configured transparency system. The implemented atomic gate is the only path that can turn that fully replayed closure into authoritative evidence. Pilot A refutation authority and the resulting `disproved` status are observed and independently audited on merge `18abdd23025cc245f0b5722a8676e5b3f1bb2237`, protected run `29860588969`, artifact `8507149183`: exact negation formalization version `1f9621d15a83cbf3c201cd6d3cc739f4624cee59fdc103cf603b94f86ebac0b9` receives accepted authoritative refutation evidence, and only a distinct reviewer's v2 `logical_negation` fidelity evidence changes the exact claim read from open to disproved.
