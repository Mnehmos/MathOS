...
pub(crate) fn validate_source_content_metadata(
    ...
    if artifact
        .semantic_metadata
        .get("source_role")
        .is_none_or(|role| role != "source_content")
    {
        return Err(source_content_metadata_error(
            "source content artifact lacks the reviewed `source_content` role",
        ));
    }
    if !matches!(
        artifact.creation_source,
        ArtifactCreationSource::UserIngest
            | ArtifactCreationSource::Import
            | ArtifactCreationSource::Migration
    ) {
        return Err(source_content_metadata_error(
            "source content artifact provenance is generated, verifier-produced, or review-produced",
        ));
    }
    if source
        .license_expression
        .as_ref()
        .is_some_and(|license| artifact.license_expression.as_deref() != Some(license.as_str()))
    {
        return Err(source_content_metadata_error(
            "source license differs from the exact content artifact license",
        ));
    }
    if source.redaction_class == RedactionClass::Public
        && source.redistribution_status == RedistributionStatus::Allowed
        && (artifact.restriction != ArtifactRestriction::Public
            || source.license_expression.is_none()
            || artifact.license_expression != source.license_expression)
    {
        return Err(source_content_metadata_error(
            "public redistributable source metadata is more permissive than its content artifact",
        ));
    }
    Ok(())
}

fn source_content_metadata_error(message: impl Into<String>) -> AppError {
    AppError::new(
...
mod tests {
    ...
    #[test]
    fn source_content_metadata_requires_reviewed_provenance_and_compatible_policy() {
        let source: SourcePayload = serde_json::from_value(json!({
            "source_type": "repository",
            "title_or_label": "Exact source fixture",
            ...
            "provenance_notes": "policy test",
            "original_text": "exact source bytes"
        }))
        .expect("source fixture decodes");
        let valid = ArtifactMetadata {
            schema_version: crate::domain::artifact::ARTIFACT_METADATA_SCHEMA_VERSION.to_owned(),
            media_type: crate::domain::ArtifactMediaType::PlainText,
        ...
        validate_source_content_metadata(&source, &valid)
            .expect("reviewed exact source metadata is accepted");

        let mut wrong_role = valid.clone();
        wrong_role.semantic_metadata.clear();
        assert_eq!(
            validate_source_content_metadata(&source, &wrong_role)
                .expect_err("unreviewed role fails")
                .code,
            "MCL_SOURCE_CONTENT_INVALID"
        );

        let mut generated = valid.clone();
        generated.creation_source = ArtifactCreationSource::Generated;
        assert_eq!(
            validate_source_content_metadata(&source, &generated)
                .expect_err("generated bytes are not source content")
                .code,
            "MCL_SOURCE_CONTENT_INVALID"
        );

        let mut license_mismatch = valid.clone();
        license_mismatch.license_expression = Some("MIT".to_owned());
        assert_eq!(
            validate_source_content_metadata(&source, &license_mismatch)
                .expect_err("source license cannot exceed exact artifact policy")
                .code,
            "MCL_SOURCE_CONTENT_INVALID"
        );

        let mut private = valid;
        private.restriction = ArtifactRestriction::Private;
        assert_eq!(
            validate_source_content_metadata(&source, &private)
                .expect_err("public redistribution cannot bind private bytes")
                .code,
            "MCL_SOURCE_CONTENT_INVALID"
        );
    }
